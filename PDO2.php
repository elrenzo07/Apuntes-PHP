...
$SECC = "%" . $_GET['secc'] . "%"; //complementar con comodines para que busque en el contenido.
$PORIG = "%" . $_GET['p_orig'] . "%";
try{
    ...
    $sql = "SELECT CODIGOARTICULO, NOMBREARTICULO, SECCION, PRECIO, PAISDEORIGEN FROM PRODUCTOS WHERE SECCION LIKE :SECC AND PAISDEORIGEN LIKE :PORIG";
    $resultado = $base->prepare($sql); //carga la instrucción.

    $resultado->execute(array(":SECC"=>$SECC,":PORIG"=>$PORIG)); //ejecuta la consulta MÚLTIPLE.
